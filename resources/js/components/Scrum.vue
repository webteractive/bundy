<template>
    <modal v-if="shown" disable-close-button>
        ...
        >
            <div
                class="bg-white w-screen h-screen md:h-auto md:w-500 relative z-20 shadow"
            >
                <ct>{{ hasPostedScrum ? "Edit" : "Add" }} Scrum</ct>

                <div class="px-4 py-6">
                    <div class="mb-4">
                        <label class="block font-bold" for="yesterday">
                            Yesterday
                        </label>
                        <p class="text-xs leading-none mb-2 italic">
                            What did you do and accomplished yesterday?
                        </p>
                        <field
                            :disabled="loading"
                    ...
                    <div class="mb-4">
                        <label class="block font-bold" for="blockers">
                            Blockers
                        </label>
                        <p class="text-xs leading-none mb-2 italic">
                            What did stop you for completing your tasks
                            yesterday?<br />Leave it empty if no blockers
                        </p>
                        <field
                            :disabled="loading"
...
export default {
    ...
    watch: {
        shown() {
            if (this.scrum !== null) {
                const { yesterday, blockers, today } = JSON.parse(
                    JSON.stringify(this.scrum)
                );
    ...
    mounted() {
        this.$store.dispatch(
            "scrum/toggle",
            this.scheduled && this.hasClockedIn && this.hasNotPostedScrum
        );
    }
};
</script>
